...
public class WikiTextParser {
	...
	private Vector<Wikilink> pageLinks = null;
	...
	public WikiTextParser(String wtext) {
		...
		if (matcher.find()) {
			redirect = true;
			if (matcher.groupCount() == 1) redirectString = matcher.group(1);
		}
	...
	public Vector<Wikilink> getLinks() {
	...
	private void parseLinks() {
		...
		while (matcher.find()) {
			...
			if (link.contains(":") == false) {
				if (temp.length > 1) {
					pageLinks.add(new Wikilink(link, temp[1]));
				} else {
					pageLinks.add(new Wikilink(link));
				}
			}
		}
	}
	...
	public InfoBox getInfoBox() {
		...
		// methods
		if (infoBox == null) infoBox = parseInfoBox();
		return infoBox;
	}
